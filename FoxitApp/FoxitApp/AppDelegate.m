...
{
    if ([[url absoluteString] hasPrefix:@"FoxitApp://fromCustomerApp"]) {
        NSURL *groupURL = [[NSFileManager defaultManager] containerURLForSecurityApplicationGroupIdentifier:AppGroupID];

        NSString *sourceFilePath = [[url absoluteString] stringByReplacingOccurrencesOfString:@"FoxitApp://fromCustomerAppfile://" withString:@""];
        NSString *docsDir = [NSHomeDirectory() stringByAppendingPathComponent:@"Documents"];
        NSString *destfilePath = [docsDir stringByAppendingPathComponent:[sourceFilePath lastPathComponent]];
